...
def run_ml_app():
	...
	with st.expander("Resultados de la Predicción Logistica"):
	...
	## Cuando he intentado el ejercicio extra dos,he cargado ambos modelos:
    ## loaded_model_pred = load_model("models/logistic_regression_model_diabetes_21_oct_2020.pkl")
	## loaded_model_tree = load_model("models/decision_tree_model_diabetes_21_oct_2020")
	## Y simplemente habria que cambiar en el nuevo expander todo lo que antes ponia prediction / prob por prediction_tree y pred_prob_tree, es decir:
	## 		prediction_tree = loaded_model.predict(single_sample)
	##	    pred_prob_tree = loaded_model.predict_proba(single_sample)
	## Pero al ejecutar me salia un error de compatibilidad, como que estaba corrupto. Asi que lo que he hecho ha sido replicar el modelo dos veces como si fuera el arbol de decision.
	## Solo que uso el mismo modelo en ambos casos.


	with st.expander("Resultados de la Predicción Arbol de decision (VER CODIGO EXPLICACION)"):
		single_sample = np.array(encoded_result).reshape(1,-1)
		...
		if prediction_tree == 1:
			st.warning("Riesgo Positivo-{}".format(prediction_tree[0]))
			pred_probability_score = {"Negativo en DM":pred_prob_tree[0][0]*100,"Positivo en DM":pred_prob_tree[0][1]*100}
			st.subheader("Valores de Predicción en probabilidades")
			st.json(pred_probability_score)
		else:
			st.success("Riesgo Negativo-{}".format(prediction_tree[0]))
			pred_probability_score = {"Negativo en DM":pred_prob_tree[0][0]*100,"Positivo en DM":pred_prob_tree[0][1]*100}
			st.subheader("Valores de Predicción en probabilidades")
			st.json(pred_probability_score)
